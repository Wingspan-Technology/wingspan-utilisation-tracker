...
export function ReportChart({ rows, clients, months, selectedClient, selectedMonth, selectedYear }: Props) {
  ...
  function buildUrl(params: { client?: string; year?: string; month?: string }) {
    const p = new URLSearchParams();
    if (params.client) p.set("client", params.client);
    if (params.year) p.set("year", params.year);
    if (params.month) p.set("month", params.month);
    const qs = p.toString();
    return `/reports/client-summary${qs ? `?${qs}` : ""}`;
  }

  function handleClientChange(slug: string) {
    router.push(buildUrl({ client: slug || undefined }));
  }

  function handleYearChange(year: string) {
    router.push(buildUrl({ client: selectedClient || undefined, year: year || undefined }));
  }

  function handleMonthChange(month: string) {
    if (month) {
      router.push(buildUrl({ client: selectedClient || undefined, month: `${currentYear}-${month}` }));
    } else {
      router.push(buildUrl({ client: selectedClient || undefined, year: currentYear || undefined }));
    }
  }
  ...
  const sorted = [...rows].sort((a, b) => b.billableDays + b.nonBillableDays - (a.billableDays + a.nonBillableDays));
  ...
  const allTasks = rows.flatMap((c) => c.projects.flatMap((p) => p.tasks));
  const maxTaskDays = Math.max(...allTasks.map((t) => t.days), 0.001);

  const totalBillable = rows.reduce((s, c) => s + c.billableDays, 0);
  const totalNonBillable = rows.reduce((s, c) => s + c.nonBillableDays, 0);
  const totalDays = totalBillable + totalNonBillable;

  return (
    <div className="space-y-6">
      <h1 className="text-2xl font-bold text-foreground">Client Summary Report</h1>
      ...
      {rows.length === 0 ? (
        ...
        <div className="space-y-4">
          {sorted.map((client) => {
            ...
            return (
              <div key={client.clientName} className="bg-card rounded-lg border border-border overflow-hidden">
                {/* Client header */}
                    ...
                    return (
                      <div key={proj.projectName}>
                        ...
                        <div className="flex items-center justify-between px-4 py-2 bg-muted/10">
                          <span className="text-sm font-medium text-foreground">{proj.projectName}</span>
                        ...
                        <div className="divide-y divide-border/50">
                          {proj.tasks.map((task) => {
                            const barWidthPct = (task.days / maxTaskDays) * 100;
                            return (
                              <div key={task.taskName} className="px-6 py-2 space-y-1.5">
